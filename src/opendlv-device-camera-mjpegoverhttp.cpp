        ...
        if ( ((0 < width) && (static_cast<uint32_t>(width) == WIDTH)) && 
            ...
            sharedMemoryARGB->setTimeStamp(ts);
            {
                ...
                if (VERBOSE) {
                    ...
                    std::clog << "[opendlv-device-mjpeg-over-http]: Acquired new frame at " << cluon::time::toMicroseconds(ts) << " microseconds." << std::endl;
                }
            }
            ...
            std::cerr << "[opendlv-device-mjpeg-over-http]: Error: expecting width " << WIDTH << ", found " << width << ", expecting height " << HEIGHT << ", found " << height << std::endl;
        }
    }
}
...
//                        std::clog << "[opendlv-device-mjpeg-over-http]: Found content-length" << expectedSizeOfJPEG << std::endl;
                    }
                    // Decode content-type.
//                    if (strncasecmp(lineFromResponseHeader, "content-type:", 13) == 0) {
//                        std::string content{lineFromResponseHeader+14};
//                        std::clog << "[opendlv-device-mjpeg-over-http]: Found content-type" << content << std::endl;
//                    }
...
int32_t main(int32_t argc, char **argv) {
    ...
    if ( (0 == commandlineArguments.count("url")) ||
        ...
        std::cerr << "Usage:   " << argv[0] << " --url=<URL> --width=<width> --height=<height> [--name.i420=<unique name for the shared memory in I420 format>] [--name.argb=<unique name for the shared memory in ARGB format>] [--verbose]" << std::endl;
        ...
        std::cerr << "Example: " << argv[0] << " --url=http://192.168.0.11?mjpeg --width=640 --height=480 --verbose" << std::endl;
    } else {
        ...
        if (!sharedMemoryI420 || !sharedMemoryI420->valid()) {
            std::cerr << "[opendlv-device-mjpeg-over-http]: Failed to create shared memory '" << NAME_I420 << "'." << std::endl;
            return retCode;
        }

        sharedMemoryARGB.reset(new cluon::SharedMemory{NAME_ARGB, WIDTH * HEIGHT * 4});
        if (!sharedMemoryARGB || !sharedMemoryARGB->valid()) {
            std::cerr << "[opendlv-device-mjpeg-over-http]: Failed to create shared memory '" << NAME_ARGB << "'." << std::endl;
            return retCode;
        }

        if ( (sharedMemoryI420 && sharedMemoryI420->valid()) &&
             (sharedMemoryARGB && sharedMemoryARGB->valid()) ) {
            std::clog << "[opendlv-device-mjpeg-over-http]: Data from camera '" << URL<< "' available in I420 format in shared memory '" << sharedMemoryI420->name() << "' (" << sharedMemoryI420->size() << ") and in ARGB format in shared memory '" << sharedMemoryARGB->name() << "' (" << sharedMemoryARGB->size() << ")." << std::endl;
